...
def save_classification_map(pred_map, filepath):
    """
    Save classification map as PNG using spectral.imshow with proper colormap.
    Matches original paper implementations (MSSGU style).

    Args:
        pred_map: 2D numpy array of predicted class labels
        filepath: Path where the PNG file will be saved
    """
    fig, ax = plt.subplots()
    numlabel = np.array(pred_map)
    h, w = numlabel.shape
    dpi = 400
    scale = 4.0
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig(filepath, format='png', transparent=True, dpi=dpi, pad_inches=0)
    plt.close()
...
def get_Samples_GT(seed: int, gt: np.array, class_count: int, train_ratio,val_ratio, samples_type: str = 'ratio', ):
    ...
    if samples_type == 'ratio':
        ...
        train_rand_idx_array = train_rand_idx  # Keep as list of lists (inhomogeneous)
        train_data_index = []
        val_data_index = []
        for c in range(len(train_rand_idx_array)):
            a = list(train_rand_idx_array[c])
    ...
    if samples_type == 'same_num':
        ...
        # Flatten lists instead of np.array() to avoid inhomogeneous shape errors
        train_data_index = []
        ...
        val_data_index = []
        for c in range(len(val_rand_idx)):
            a = val_rand_idx[c]
            for j in range(len(a)):
